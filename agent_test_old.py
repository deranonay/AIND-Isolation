...
class Project1Test(unittest.TestCase):
    ...
    def initAUT(self, depth, eval_fn, iterative=False,
        ...
        """
        reload(game_agent)
        agentUT = game_agent.MinimaxPlayer(depth, eval_fn) if method == 'minimax' else game_agent.AlphaBetaPlayer(depth, eval_fn)
    ...
    def test_alphabeta_interface(self):
        ...
        for move in board.get_legal_moves():
            next_state = board.forecast_move(move)
            v, _ = agentUT.minimax_with_score(next_state, test_depth, True, apply_alphabeta=True)
    ...
    def test_alphabeta(self):
        ...
        for idx in range(len(counts)):
            ...
            # disable search timeout by returning a constant value
            agentUT.time_left = lambda: 1e3
            _, move = agentUT.minimax_with_score(board, test_depth, True, apply_alphabeta=True)
